...
export default function BlogListingPage() {
  ...
  const availableTags = useMemo(
    () => normalizeTags(tagResponse?.payload?.data ?? []),
    [tagResponse?.payload?.data]
  );

  const allBlogs = blogPage?.payload?.data ?? [];
  ...
  const pagination = blogPage?.payload?.pagination;
        ...
        <section className="container mx-auto px-4 pb-12">
          ...
          {isLoading ? (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {Array.from({ length: HERO_POSTS }).map((_, index) => (
